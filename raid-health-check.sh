#!/bin/bash
if [[ `/opt/MegaRAID/MegaCli/MegaCli64 -AdpAllInfo -aAll | grep "Critical Disk" | awk {'print $4'}` -eq 0 ]];
then
...
if [[ `/opt/MegaRAID/MegaCli/MegaCli64 -AdpAllInfo -aAll | grep "Failed Disk" | awk {'print $4'}` -eq 0 ]];
then
...
if [ `/opt/MegaRAID/MegaCli/MegaCli64 -AdpAllInfo -aAll | grep "Critical Disk" | awk {'print $4'}` -eq 0 ] && [ `/opt/MegaRAID/MegaCli/MegaCli64 -AdpAllInfo -aAll | grep "Failed Disk" | awk {'print $4'}` -eq 0 ] ;
then
    #SEND MAIL
